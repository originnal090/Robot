...
namespace UnityEditor.XR.Interaction.Toolkit.Samples
{
    ...
    class StarterAssetsSampleProjectValidation
    {
        ...
        static int s_SettingsRetryCount;
        ...
        {
            var interactionLayerSettings = InteractionLayerSettings.Instance;
            if (interactionLayerSettings == null)
            {
                if (s_SettingsRetryCount++ < 3)
                {
                    EditorApplication.delayCall += AddRulesAndRunCheck;
                    return;
                }

                Debug.LogWarning("Starter Assets project validation skipped because Interaction Layer Settings could not be loaded.");
                return;
            }

            // A newly created settings instance may not have initialized its layer names yet.
            interactionLayerSettings.OnBeforeSerialize();
            ...
            if (s_BuildValidationRules.Count == 0)
            {
                s_BuildValidationRules.Add(
                    new BuildValidationRule
                    {
                        ...
                        FixItAutomatic = interactionLayerSettings.IsLayerEmpty(k_TeleportLayerIndex) || IsInteractionLayerTeleport(),
                        Error = false,
        ...
        static bool IsInteractionLayerTeleport()
        {
            ...
            return settings != null && string.Equals(settings.GetLayerNameAt(k_TeleportLayerIndex), k_TeleportLayerName, StringComparison.OrdinalIgnoreCase);
        }
