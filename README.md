...
- **桌面操作台**：实时画面与遥测、运行中热调检测/控制参数、手动行为按钮
  （点头/摇头/三档俯仰/点动）、有界日志；视频用纯 Python MJPEG 解析，FFmpeg 崩溃免疫。
- **手柄遥操作**：PC 端直接读游戏手柄——Windows 下优先走原生 XInput（ctypes，
  零依赖，与 joy.cpl 同通路），pygame 为备选/跨平台后端；LS 前后/横移，RS
  旋转/三档俯仰；**B 键抢断**——自主寻路运行中按 B 立即切入手动控制，再按 B 恢复
...
- **左摇杆（LS）**：上下 = 前进/后退，左右 = 左右横移。
- **右摇杆（RS）**：左右 = 原地旋转且不改变头部；上下每次越过 0.20 死区移动
  一个俯仰档位。低头时连续两次“回中后推 RS↑”依次到回正、抬头；RS↓ 反向。
  摇杆只在**手动模式**生效，误碰不会打断自主。
...
| GUI 点动/停止/闭环控制 | ✓ | ✓ |
| `CMD:nod` / `CMD:shake` / `CMD:head_down` / `CMD:head_center` / `CMD:head_up` | Unity 记录动作并预览三档俯仰 | ✓ |
| `ACTION_V1` 单动作 ID、accepted/done 回执 | ✓（0.2.0） | ✓ |
| 超声波距离遥测（`DIST` 行，避障必需） | ✗ | ✓ |
